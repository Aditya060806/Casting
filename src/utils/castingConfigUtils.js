...
async function createCLIConfigFile() {
  try {
    ...
    const answers = await prompt(castingConfigQuestions);

    outputFileSync('casting.json', JSON.stringify(answers, null, 2));

    blank();
    success('Created the casting.json config file');
    blank();
    ...
    error('Could not create config file', {
      details: 'Failed to write casting.json',
      suggestions: [
...
async function updateCLIConfigFile(missingConfigQuestions, currentConfigFile) {
  try {
    ...
    blank();
    success('Updated the casting.json config file');
    blank();
    outro('You can always update the config file manually. Happy Hacking!');
...
export async function getCLIConfigFile() {
  ...
  try {
    ...
    try {
      accessSync('./casting.json', constants.R_OK);
      const currentConfigFile = JSON.parse(
        readFileSync('./casting.json'),
      );

      /**
       *  Check to see if there's a difference between castingConfigQuestions and the currentConfigFile.
       *  If there is, update the currentConfigFile with the missingConfigQuestions.
       */

      const missingConfigQuestions = castingConfigQuestions.filter(
        question =>
